...
namespace KWin
{
    ...
    std::unique_ptr<BlurManagerInterface> BlurEffect::s_blurManager = nullptr;
    std::unique_ptr<QTimer> BlurEffect::s_blurManagerRemoveTimer = nullptr;
    ...
    BlurEffect::BlurEffect()
    {
        ...
        {
            if (s_blurManagerRemoveTimer == nullptr)
            {
                s_blurManagerRemoveTimer = std::make_unique<QTimer>(QCoreApplication::instance());
                s_blurManagerRemoveTimer->setSingleShot(true);
                s_blurManagerRemoveTimer->callOnTimeout([]() -> void
                                                        {
                    if (s_blurManager != nullptr) {
                        auto manager = std::move(s_blurManager);
                        manager->remove();
                        // manager é destruído automaticamente ao sair do escopo
                    } });
            }
            s_blurManagerRemoveTimer->stop();
            if (s_blurManager == nullptr)
            {
                s_blurManager = std::make_unique<BlurManagerInterface>(effects->waylandDisplay(), s_blurManagerRemoveTimer.get());
            }
        }
    ...
    void BlurEffect::updateBlurRegion(EffectWindow *w)
    {
        ...
        if (net_wm_blur_region != XCB_ATOM_NONE)
        {
            ...
            Region region;
            // Verificação robusta de bounds: cada retângulo precisa de 4 valores (x, y, w, h)
        ...
        if (surf && surf->blur())
        {
            content = Region(surf->blur()->region());
        }
        ...
        if (auto internal = w->internalWindow())
        {
            ...
            if (property.isValid())
            {
                content = Region(property.value<QRegion>());
            }
        }
    ...
    Region BlurEffect::decorationBlurRegion(const EffectWindow *w) const
    {
        ...
        Region decorationRegion = Region(QRegion(w->decoration()->rect().toAlignedRect())) - Region(QRegion(w->contentsRect().toRect()));
    ...
    Region BlurEffect::blurRegion(EffectWindow *w) const
    {
        ...
        if (auto it = m_windows.find(w); it != m_windows.end())
        {
            ...
            {
                if (content->rects().empty())
                {
                    // An empty region means that the blur effect should be enabled
                    // for the whole window.
                    region = Region(QRegion(w->contentsRect().toRect()));
                }
                else
                {
                    region = content->translated(w->contentsRect().topLeft().toPoint()) & Region(QRegion(w->contentsRect().toRect()));
                }
                if (frame.has_value())
                {
                    region += frame.value();
            ...
            m_helper->roundBlurRegion(w, &qregion);
            region = Region(qregion);
        }
    ...
    void BlurEffect::prePaintScreen(ScreenPrePaintData &data, std::chrono::milliseconds presentTime)
    {
        ...
        m_currentScreen = effects->waylandDisplay() ? reinterpret_cast<LogicalOutput *>(data.screen) : nullptr;
    ...
    GLTexture *BlurEffect::ensureNoiseTexture()
    {
        ...
        if (!m_noisePass.noiseTexture || m_noisePass.noiseTextureScale != scale || m_noisePass.noiseTextureStength != m_noiseStrength)
        {
            ...
            std::mt19937 gen(rd());
            std::uniform_int_distribution<int> dis(0, m_noiseStrength - 1);
    ...
    void BlurEffect::blur(const RenderTarget &renderTarget, const RenderViewport &viewport, EffectWindow *w, int mask, const Region &region, WindowPaintData &data)
    {
        ...
        QRegion blurShape = calculateBlurShape(w, data);
        ...
        if (region != Region::infinite())
        {
            ...
            {
                const QRectF deviceClipRect = snapToPixelGridF(scaledRect(QRect(clipRect.x(), clipRect.y(), clipRect.width(), clipRect.height()), viewport.scale()))
        ...
        updateOffscreenTargets(backgroundRect.size(), textureFormat, renderInfo);
        if (renderInfo.textures.empty())
        {
            return;
        }

        // Fetch the pixels behind the shape that is going to be blurred.
        const QRegion dirtyRegion = QRegion(region & backgroundRect);
    ...
    QRegion BlurEffect::calculateBlurShape(EffectWindow *w, const WindowPaintData &data) const
    {
        QRegion blurShape = QRegion(blurRegion(w)).translated(w->pos().toPoint());
    ...
    void BlurEffect::updateOffscreenTargets(const QSize &size, GLenum format, BlurRenderData &renderInfo)
    {
        if (renderInfo.framebuffers.size() != (m_iterationCount + 1) || renderInfo.textures[0]->size() != size || renderInfo.textures[0]->internalFormat() != format)
        {
            ...
            {
                auto texture = GLTexture::allocate(format, size / (1 << i));
                if (!texture)
                {
